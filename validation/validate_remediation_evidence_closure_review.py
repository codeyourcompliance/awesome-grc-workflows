...
REQUIRED = [
    "README.md", "scenario.md", "task-brief.md", "scoring-rubric.md",
    "reviewer-guide.md", "reference-answer.md", "role-relevance.md", "metadata.yaml",
    "inputs/README.md", "inputs/01-issue-record.md", "inputs/02-closure-requirements.csv",
    "inputs/03-remediation-action-plan.md", "inputs/04-closure-submission.md",
    "inputs/05-evidence-inventory.csv", "inputs/06-change-record.md",
    "inputs/07-post-change-sample-test.md", "inputs/08-monitoring-report.csv",
    "inputs/09-management-representation.md", "inputs/10-approval-history.csv",
    "inputs/11-account-removal-record.md", "templates/evidence-review-workpaper.csv",
    "templates/deficiency-register.csv", "templates/closure-readiness-memo.md",
]
BANNED = [
...
def main() -> int:
    ...
    markers = {
        "README.md": [DISCLAIMER, "## Bounded work action", "## Authority boundary", "## Relevant public resources"],
        "scenario.md": ["# Synthetic Scenario", "Cedar Quay Transit Cooperative", "## Roles and authority", DISCLAIMER],
        "task-brief.md": ["## Required tools", "## Required deliverables", "### Permitted assumptions", "### Prohibited assumptions", "### When evidence is insufficient", "## Excluded scope"],
        "scoring-rubric.md": ["Total: 100 points", "## Critical errors", "Authority-boundary discipline"],
        "reviewer-guide.md": ["## Mandatory observations", "## Designed non-issues or false positives", "## Intentional ambiguities", "## Acceptable answer range", "## Professional judgment", "## Authority checks", "## Blocking errors", "## Minimum passing evidence", "## Partial credit"],
        "reference-answer.md": ["one defensible response", "## Preferred answer", "## Acceptable alternatives", "## Unsupported answers", "## Assumptions that change the result", "## Evidence needed to resolve uncertainty", "## Decisions reserved for authorized roles"],
        "metadata.yaml": [f"id: {CASE_ID}", "status: case-ready", "version: 1.0.0", "synthetic_data: true", "private_dependency: false", "maintainer_service_required: false", "pilot_completions: 0"],
    }
    ...
    if hashes.count("sha256-a91f") != 2:
        errors.append("evidence inventory: designed duplicate hash missing")
    ...
    if not any(r.get("object") == "Privileged Access Procedure" and r.get("version") == "1.9" for r in approvals):
    ...
    if "This recommendation is not final issue-closure approval." not in docs.get("templates/closure-readiness-memo.md", ""):
        errors.append("memo template: authority statement missing")

    role_doc = docs.get("role-relevance.md", "")
    allowed_relationships = {"performs", "contributes", "reviews", "consumes", "authorizes/owns"}
